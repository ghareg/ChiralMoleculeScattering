...
void updateLVal(LValStruct& LVal, int n, int m, const Param& pm, const PauliMatrix& pal, const double* fact)
{
	...
	LVal.L2 = (F2 * I2 + F3 * I3) * pal.pal0 + alphaSOC1 * (2.0 * pm.kCosal * I3 + (F3 * pm.kCosal - F1 * pm.kSinal * pm.sinBeta) * I1)  * pal.pal1 -
		alphaSOC2 * (2.0 * pm.kCosal * I2 + (F2 * pm.kCosal - F1 * pm.kSinal * pm.cosBeta) * I1) * pal.pal2 + 
		alphaSOC3 * (2.0 * pm.kSinal * (I2 * pm.sinBeta - I3 * pm.cosBeta) + (F2 * pm.kSinal * pm.sinBeta - F3 * pm.kSinal * pm.cosBeta) * I1) * pal.pal3;
}	
